...
function urlBuilder(type, location) {
  return `http://api.wunderground.com/api/${APIKEY}/${type}/q/${location.lat},${
    location.lng
  }.json`;
}
  ...
  https.get(geoipURI, res => {
    ...
    res.on("end", () => {
      try {
        ...
        const weatherURL = urlBuilder(
          "conditions/forecast/astronomy",
          location
        );
...
const renderWeather = data => {
  // console.log(data);
  let locationAndTime = `Showing current weather in ${
    data.display_location.full
  } at ${data.observation_time_rfc822}.`;
  let temperature = `- The temperature is ${data.temp_c}°C and feels like ${
    data.feelslike_c
  }°C`;
  let humidity = `- The humidity is ${data.relative_humidity}`;
  let windSpeed = `- The wind is ${
    data.wind_string
  } and blowing at a speed of ${data.wind_kph} km/hrs from ${data.wind_dir}`;
  let visibility = `- The visibility is ${data.visibility_km} kms`;
  const pressure = `- Current pressure is ${data.pressure_mb} mbar`;
  const precipiation = `- There is a ${
    data.precip_today_metric
